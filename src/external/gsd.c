...
enum { GSD_INITIAL_NAME_BUFFER_SIZE = 1024 };

/// Size of initial frame index
enum { GSD_INITIAL_FRAME_INDEX_SIZE = 16 };

/// Size of write buffer
enum { GSD_WRITE_BUFFER_SIZE = 16 * 1024 * 1024 };

/// Size of copy buffer
enum { GSD_COPY_BUFFER_SIZE = 128 * 1024 };

/// Size of hash map
enum { GSD_NAME_MAP_SIZE = 57557 };

/// Current GSD file specification
enum { GSD_CURRENT_FILE_VERSION = 2 };
...
inline ssize_t pread(int fd, void* buf, size_t count, int64_t offset) {
  ...
  if (count > UINT_MAX) return GSD_ERROR_IO;
...
inline static int gsd_byte_buffer_append(struct gsd_byte_buffer* buf, const char* data, size_t size) {
  ...
  if (buf->size + size > buf->reserved) {
    ...
    gsd_util_zero_memory(buf->data + (buf->size + size), sizeof(char) * (new_reserved - (buf->size + size)));
...
inline static int gsd_index_buffer_map(struct gsd_index_buffer* buf, struct gsd_handle* handle) {
  ...
  buf->mapped_data = mmap(NULL, index_size + (handle->header.index_location - offset), PROT_READ, MAP_SHARED, handle->fd, offset);

  if (buf->mapped_data == MAP_FAILED) {
    return GSD_ERROR_IO;
  }

  buf->data = (struct gsd_index_entry*)(((char*)buf->mapped_data) + (handle->header.index_location - offset));
  ...
  ssize_t bytes_read = gsd_io_pread_retry(
      handle->fd, buf->data, sizeof(struct gsd_index_entry) * handle->header.index_allocated_entries, handle->header.index_location);
...
inline static int gsd_index_buffer_add(struct gsd_index_buffer* buf, struct gsd_index_entry** entry) {
  ...
  if (buf->size == buf->reserved) {
    ...
    // zero the new memory
    gsd_util_zero_memory(buf->data + buf->reserved, sizeof(struct gsd_index_entry) * (new_reserved - buf->reserved));
    buf->reserved = new_reserved;
  }
...
inline static int gsd_cmp_index_entry(const struct gsd_index_entry* a, const struct gsd_index_entry* b) {
  int result = 0;
...
inline static int gsd_expand_file_index(struct gsd_handle* handle, size_t size_required) {
  ...
  while (total_bytes_written < old_index_bytes) {
    ...
    ssize_t bytes_read = gsd_io_pread_retry(handle->fd, buf, bytes_to_copy, old_index_location + total_bytes_written);
    ...
    ssize_t bytes_written = gsd_io_pwrite_retry(handle->fd, buf, bytes_to_copy, new_index_location + total_bytes_written);
  ...
  ssize_t bytes_written = gsd_io_pwrite_retry(handle->fd, &(handle->header), sizeof(struct gsd_header), 0);
...
inline static int gsd_flush_write_buffer(struct gsd_handle* handle) {
  ...
  ssize_t bytes_written = gsd_io_pwrite_retry(handle->fd, handle->write_buffer.data, handle->write_buffer.size, offset);
...
inline static int gsd_flush_name_buffer(struct gsd_handle* handle) {
  ...
  int retval = gsd_byte_buffer_append(&handle->file_names.data, handle->frame_names.data.data, handle->frame_names.data.size);
  if (retval != GSD_SUCCESS) {
    return retval;
  }
  ...
  if (handle->file_names.data.reserved > old_reserved) {
    ...
    uint64_t offset = handle->file_size;
    ssize_t bytes_written = gsd_io_pwrite_retry(handle->fd, handle->file_names.data.data, handle->file_names.data.reserved, offset);
    ...
    // write the new header out
    bytes_written = gsd_io_pwrite_retry(handle->fd, &(handle->header), sizeof(struct gsd_header), 0);
    if (bytes_written != sizeof(struct gsd_header)) {
      return GSD_ERROR_IO;
    }
  } else {
    ...
    ssize_t bytes_written = gsd_io_pwrite_retry(handle->fd, handle->file_names.data.data + old_size,
                                                handle->file_names.data.reserved - old_size, offset + old_size);
...
*/
inline static int gsd_initialize_file(int fd, const char* application, const char* schema, uint32_t schema_version) {
...
inline static int gsd_initialize_handle(struct gsd_handle* handle) {
  ...
  bytes_read = gsd_io_pread_retry(handle->fd, handle->file_names.data.data, namelist_n_bytes, handle->header.namelist_location);
...
int gsd_create_and_open(struct gsd_handle* handle, const char* fname, const char* application, const char* schema,
                        uint32_t schema_version, const enum gsd_open_flag flags, int exclusive_create) {
  // zero the handle
...
int gsd_truncate(struct gsd_handle* handle) {
  ...
  retval = gsd_initialize_file(handle->fd, old_header.application, old_header.schema, old_header.schema_version);
...
int gsd_end_frame(struct gsd_handle* handle) {
  ...
  if (handle->frame_index.size > 0) {
    ...
    int64_t write_pos = handle->header.index_location + sizeof(struct gsd_index_entry) * handle->file_index.size;

    size_t bytes_to_write = sizeof(struct gsd_index_entry) * handle->frame_index.size;
    ssize_t bytes_written = gsd_io_pwrite_retry(handle->fd, handle->frame_index.data, bytes_to_write, write_pos);
    ...
    memcpy(handle->file_index.data + handle->file_index.size, handle->frame_index.data,
...
int gsd_write_chunk(struct gsd_handle* handle, const char* name, enum gsd_type type, uint64_t N, uint32_t M, uint8_t flags,
                    const void* data) {
  // validate input
  if (N > 0 && data == NULL) {
    return GSD_ERROR_INVALID_ARGUMENT;
  ...
  if (handle->header.gsd_version < gsd_make_version(2, 0)) {
    if (handle->file_index.size > 0) {
      ...
      ssize_t bytes_written
          = gsd_io_pwrite_retry(handle->fd, buf.data, sizeof(struct gsd_index_entry) * buf.size, handle->header.index_location);
    ...
    if (handle->file_names.n_names > 0) {
      ...
      ssize_t bytes_written
          = gsd_io_pwrite_retry(handle->fd, new_name_buf.data, new_name_buf.reserved, handle->header.namelist_location);
